...
//   const HeaderTabText = [`Notification ${notificationCnt}`, "Profile", "General Settings"];
    ...
    const { user } = useAuth();
    // const [notificationData,setNotificationData]  = useRecoilState(notificationAtom);
    ...
    const Source = [
      ...
      const updateChkStatus = async (id) => {
        const apiResponse = await updateNotificationStatus(id,user.e_mail,user.cmpid);
        if (!apiResponse.hasError) {
  ...
  return (
    ...
    <HeaderTab HeaderTabText={HeaderTabText} HeaderText={activePage} setActivePage={setActivePage} NotificationCnt={notificationCnt}/>
